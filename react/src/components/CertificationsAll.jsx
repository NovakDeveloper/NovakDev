...
const CertificationsAll = ({openAllCertifications, onClick}) => {
  ...
  return (openAllCertifications &&
    <div className='fixed top-0 left-0 w-full h-full px-4 md:p-16 z-10 bg-opacity-black' ref={certificationsRef}>
      <div className='wrapper bg-almost-white border border-dark-green rounded h-full overflow-hidden flex flex-col'>
      <span onClick={handleCloseButtonClick} id="close-icon" className='absolute top-4 md:top-8 right-8 text-[2rem] border border-dark-green rounded-full bg-almost-white w-[3rem] h-[3rem] flex justify-center items-center cursor-pointer'>
      ...
      <p className='text-x-large xxl:text-[1.875rem] leading-14 px-4'>Here, you can explore all the certifications I have acquired throughout my professional journey.</p>
      <div className={`wrapper h-full relative overflow-auto`}>
      <div className={`certifications w-full grid gap-4 sm:grid-cols-2 md:grid-cols-4 absolute top-0 left-0 p-4 md:p-8`}>
